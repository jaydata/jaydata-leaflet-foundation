...
$data.Class.define("$data.storageProviders.sqLite.SqLiteStorageProvider",$data.StorageProviderBase,null,{constructor:function(a,b){this.SqlCommands=[];this.context=b;this.providerConfiguration=$data.typeSystem.extend({databaseName:"JayDataDemo",version:"",displayName:"JayData demo db",maxSize:1048576,dbCreation:$data.storageProviders.DbCreationType.DropTableIfChanged},a);this.providerName="";for(var c in $data.RegisteredStorageProviders)$data.RegisteredStorageProviders[c]===this.getType()&&(this.providerName=
c);this.context&&this.context._buildDbType_generateConvertToFunction&&this.buildDbType_generateConvertToFunction&&(this.context._buildDbType_generateConvertToFunction=this.buildDbType_generateConvertToFunction);this.context&&this.context._buildDbType_modifyInstanceDefinition&&this.buildDbType_modifyInstanceDefinition&&(this.context._buildDbType_modifyInstanceDefinition=this.buildDbType_modifyInstanceDefinition)},_createSqlConnection:function(){var a={fileName:this.providerConfiguration.databaseName,
...
first:{},include:{}},enumerable:!0,writable:!0},initializeStore:function(a){a=$data.typeSystem.createCallbackSetting(a);this.context._storageModel.forEach(function(a){this.SqlCommands.push(this.createSqlFromStorageModel(a)+" ")},this);var b=this._createSqlConnection(),c=this;b.createCommand("SELECT * FROM sqlite_master WHERE type = 'table'",null).executeQuery({success:function(d){for(var e={},g=0;g<d.rows.length;g++){var f=d.rows[g];e[f.tbl_name]=f}switch(c.providerConfiguration.dbCreation){case $data.storageProviders.DbCreationType.Merge:Guard.raise(new Exception("Not supported db creation type"));
break;case $data.storageProviders.DbCreationType.DropTableIfChanged:d=[];for(g=0;g<c.SqlCommands.length;g++)if(""!=c.SqlCommands[g]){var h=RegExp("^CREATE TABLE IF NOT EXISTS ([^ ]*) (\\(.*\\))","g").exec(c.SqlCommands[g]);if(h&&(f=h[1],h=h[2],e[f.slice(1,f.length-1)])){var i=RegExp("\\(.*\\)","g"),i=e[f.slice(1,f.length-1)].sql.match(i);(!i||h.toLowerCase()!=i[0].toLowerCase())&&d.push("DROP TABLE IF EXISTS ["+e[f.slice(1,f.length-1)].tbl_name+"];")}}c.SqlCommands=c.SqlCommands.concat(d);break;case $data.storageProviders.DbCreationType.DropAllExistingTables:for(h in e)h&&
!h.match("^__")&&!h.match("^sqlite_")&&c.SqlCommands.push("DROP TABLE IF EXISTS ["+e[h].tbl_name+"];")}c._runSqlCommands(b,{success:a.success,error:a.error})},error:a.error})},executeQuery:function(a,b){var b=$data.typeSystem.createCallbackSetting(b),c=this._createSqlConnection(),d=this._compile(a);a.actionPack=d.actions;a.sqlConvertMetadata=d.converter;a.modelBinderConfig=d.modelBinderConfig;c.createCommand(d.sqlText,d.params).executeQuery({success:function(c){b.success&&(a.rawDataList=c.rows,b.success(a))},
error:b.error})},_compile:function(a){var b=new $data.storageProviders.sqLite.SQLiteCompiler,a=b.compile(a);a.hasSelect=null!=b.select;return a},getTraceString:function(a){return this._compile(a)},_runSqlCommands:function(a,b){if(this.SqlCommands&&0<this.SqlCommands.length){var c=this.SqlCommands.pop(),d=this;a.createCommand(c,null).executeQuery({success:function(){d._runSqlCommands.apply(d,[a,b])},error:b.error})}else b.success(this.context)},setContext:function(a){this.context=a},saveChanges:function(a,
b){var c=this._createSqlConnection(),d=this.buildIndependentBlocks(b);this.saveIndependentBlocks(b,d,c,a)},saveIndependentBlocks:function(a,b,c,d){function e(){if(0===f.length)d.success();else{var a=f.shift().map(function(a){var b=g.context._storageModel.getStorageModel(a.data.getType()).PhysicalType;a.physicalData=b.convertTo(a.data);return a},this);try{g.saveIndependentItems(a,c,{success:function(){g.postProcessItems(a);e()},error:d.error})}catch(b){d.error(b)}}}var g=this,f=[].concat(b);e()},saveIndependentItems:function(a,
b,c){function d(a,b){var c=[],d=[];b.forEach(function(a,b){if(a&&(a.query&&(c[b]=a.query),a.param))d[b]=a.param});return a.createCommand(c,d)}var e=this,g=a.map(function(a){return e.saveEntitySet(a)}),g=g.filter(function(a){return a});0===g.length?c.success(a):d(b,g).executeQuery({success:function(g){g=g.map(function(c,d){return c&&c.insertId?e.save_reloadSavedEntity(c.insertId,a[d].entitySet.tableName,b):null});g=d(b,g);0<g.query.length?g.executeQuery(function(b){b.forEach(function(b,c){b&&b.rows&&
(a[c].physicalData.initData=b.rows[0])});c.success(a)}):c.success(0)},error:c.error})},postProcessItems:function(a){function b(a){var b=a.name;if(c.hasOwnProperty(b))return c[b];a=a.memberDefinitions.getPublicMappedProperties().filter(function(a){return a.computed});return c[b]=a}var c={};a.forEach(function(a){a.physicalData&&b(a.data.getType()).forEach(function(b){a.data[b.name]=a.physicalData[b.name]},this)},this)},saveEntitySet:function(a){switch(a.data.entityState){case $data.EntityState.Added:return this.save_NewEntity(a);
case $data.EntityState.Deleted:return this.save_DeleteEntity(a);case $data.EntityState.Modified:return this.save_UpdateEntity(a);case $data.EntityState.Unchanged:break;default:Guard.raise(new Exception("Not supported entity state"))}},save_DeleteEntity:function(a){for(var b="DELETE FROM ["+a.entitySet.tableName+"] WHERE(",c=!1,d=!1,e=[];!c;)a.physicalData.constructor.memberDefinitions.getPublicMappedProperties().forEach(function(g){c&&!b.match(" AND $")&&(b+=" AND ");if(g.key||d){b+="(["+g.name+"] == ?)";
var f=a.data.getType().memberDefinitions.getMember(g.name);f&&f.converter&&f.converter[this.providerName]&&"function"==typeof f.converter[this.providerName].toDb?e.push(f.converter[this.providerName].toDb(a.data[f.name],f,this.context,f.dataType)):e.push(this.fieldConverter.toDb[Container.resolveName(g.dataType)](a.data[g.name]));c=!0}},this),c||(d=!0);b.match(" AND $")&&(b=b.slice(0,b.length-5));b+=");";return{query:b,param:e}},save_UpdateEntity:function(a){var b=" SET ",c="WHERE(",d=!1,e=[],g=[];
a.physicalData.constructor.memberDefinitions.getPublicMappedProperties().forEach(function(f){if(void 0!==a.physicalData[f.name])if(d&&!c.match(" AND $")&&(c+=" AND "),5<b.length&&!b.match(",$")&&(b+=","),f.key){c+="(["+f.name+"] == ?)";var h=a.data.getType().memberDefinitions.getMember(f.name);h&&h.converter&&h.converter[this.providerName]&&"function"==typeof h.converter[this.providerName].toDb?e.push(h.converter[this.providerName].toDb(a.physicalData[h.name],f,this.context,h.dataType)):e.push(this.fieldConverter.toDb[Container.resolveName(f.dataType)](a.physicalData[f.name]));
d=!0}else b+="["+f.name+"] = ?",(h=a.data.getType().memberDefinitions.getMember(f.name))&&h.converter&&h.converter[this.providerName]&&"function"==typeof h.converter[this.providerName].toDb?g.push(f.converter[this.providerName].toDb(a.physicalData[h.name],h,this.context,h.dataType)):g.push(this.fieldConverter.toDb[Container.resolveName(f.dataType)](a.physicalData[f.name]))},this);d||Guard.raise(new Exception("Not supported UPDATE function without primary key!"));c.match(" AND $")&&(c=c.slice(0,c.length-
5));b.match(",$")&&(b=b.slice(0,b.length-1));return{query:"UPDATE ["+a.entitySet.tableName+"]"+b+" "+c+");",param:g.concat(e)}},save_NewEntity:function(a){var b="INSERT INTO ["+a.entitySet.tableName+"](",c="",d="",e=[];a.physicalData.constructor.memberDefinitions.getPublicMappedProperties().forEach(function(b){if(b.key&&!b.computed&&Object.isNullOrUndefined(a.physicalData[b.name]))Guard.raise(new Exception("Key is not set","Value exception",a));else{0<c.length&&","!=c[c.length-1]&&(c+=",",d+=",");
var f=b.name;if(void 0!==a.physicalData[f]&&b.dataType&&(!b.dataType.isAssignableTo||b.dataType.isAssignableTo&&!b.dataType.isAssignableTo($data.EntitySet))){d+="?";c+="["+f+"]";var h=a.data.getType().memberDefinitions.getMember(b.name);h&&h.converter&&h.converter[this.providerName]&&"function"==typeof h.converter[this.providerName].toDb?e.push(h.converter[this.providerName].toDb(a.physicalData[f],h,this.context,h.dataType)):e.push(this.fieldConverter.toDb[Container.resolveName(b.dataType)](a.physicalData[f]))}}},
this);1>e.length?b="INSERT INTO ["+a.entitySet.tableName+"] Default values":(","==c[c.length-1]&&(c=c.slice(0,c.length-1)),","==d[d.length-1]&&(d=d.slice(0,d.length-1)),b+=c+") VALUES("+d+");");return{query:b,param:e}},save_reloadSavedEntity:function(a,b){return{query:"SELECT * FROM "+b+" WHERE rowid=?",param:[a]}},createSqlFromStorageModel:function(a){(void 0===a||null===a||void 0===a.PhysicalType)&&Guard.raise("StorageModel not contains physical entity definition");var b=0,c=0;a.PhysicalType.memberDefinitions.getPublicMappedProperties().forEach(function(a){a.key&&
b++;a.computed&&c++},this);1===c&&1<b&&Guard.raise(new Exception("Do not use computed field with multiple primary key!"));1<c&&1<b&&Guard.raise(new Exception("Do not use multiple computed field!"));var d="CREATE TABLE IF NOT EXISTS ["+a.TableName+"] (",e=",PRIMARY KEY (";a.PhysicalType.memberDefinitions.getPublicMappedProperties().forEach(function(b,f){0<f&&!d.match(", $")&&!d.match("\\($")&&(d+=", ");d+=this.createSqlFragmentFromField(b,1===c,a);0===c&&b.key&&(14<e.length&&!e.match(", $")&&(e+=", "),
e+="["+b.name+"]")},this);d.match(", $")&&(d=d.substr(0,d.length-2));0===c&&14<e.length&&(d+=e+")");return d+=");"},createSqlFragmentFromField:function(a,b,c){return"schemaCreate"in a&&a.schemaCreate?a.schemaCreate(a):(new this.FieldTypeBuilder(a,this,b,c)).build()},FieldTypeBuilder:function(a,b,c,d){this.fieldDef="";this.fld=a;this.provider=b;this.parsePk=c;this.entitySet=d;this.build=function(){switch(Container.resolveType(this.fld.dataType)){case $data.Array:case $data.String:case $data.Guid:case "text":case "string":case $data.GeographyPoint:case $data.GeographyLineString:case $data.GeographyPolygon:case $data.GeographyMultiPoint:case $data.GeographyMultiLineString:case $data.GeographyMultiPolygon:case $data.GeographyCollection:case $data.GeometryPoint:case $data.GeometryLineString:case $data.GeometryPolygon:case $data.GeometryMultiPoint:case $data.GeometryMultiLineString:case $data.GeometryMultiPolygon:case $data.GeometryCollection:this.buildFieldNameAndType("TEXT");

...
b=[],c;for(c in this.context._entitySetReferences)b.push(this.context._entitySetReferences[c].tableName);var e=null;if(this.providerConfiguration.persistentData&&window.localStorage)var d=this,e=JSON.parse(window.localStorage.getItem(this.providerConfiguration.databaseName||"JayData_InMemory_Provider"),function(a,c){return-1<b.indexOf(a)&&c.map?c.map(function(b){return new d.context[a].createNew(b)}):c});e=e||this.providerConfiguration.source||{};this.providerConfiguration.source={inmemory_sequence:{}};
for(var f=0;f<this.context._storageModel.length;f++){var g=this.context._storageModel[f];this.providerConfiguration.source[g.TableName]=[];var h=g.LogicalType.memberDefinitions.getKeyProperties().filter(function(a){return a.computed});1<h.length&&Guard.raise(new Exception("More than one computed field not supported in "+g.TableName+" entity set."));var j=!1;1===h.length&&(c=Container.resolveType(h[0].type),c===$data.Integer?(this.providerConfiguration.source.inmemory_sequence[g.TableName]=0,j=!0):
c!==$data.Guid&&Guard.raise(new Exception("Not supported key field type. Computed pk field type are $data.Integer or $data.Guid!","ComputedKeyFieldError")));if(e[g.TableName])for(c=0;c<e[g.TableName].length;c++){var i=e[g.TableName][c];if(i instanceof g.LogicalType){if(j){var k=i[h[0].name];k>this.providerConfiguration.source.inmemory_sequence[g.TableName]&&(this.providerConfiguration.source.inmemory_sequence[g.TableName]=k)}this.providerConfiguration.source[g.TableName].push(i)}else Guard.raise(new Exception("Invalid element in source: "+
g.TableName))}}a.success(this.context)},executeQuery:function(a,b){var b=$data.typeSystem.createCallbackSetting(b),c;try{c=this._compile(a)}catch(e){b.error(e);return}var d=[].concat(this.providerConfiguration.source[a.context.getEntitySetFromElementType(a.defaultType).tableName]||[]);c.$filter&&!c.$every&&(d=d.filter(c.$filter));c.$map&&0===Object.keys(a.modelBinderConfig).length&&(d=d.map(c.$map));if(c.$order&&0<c.$order.length){c.$order.reverse();for(var f=0,g=c.$order.length;f<g;f++)c.$order[f].ASC?
d.sort(function(a,b){var d=c.$order[f](a),e=c.$order[f](b);return d===e?0:d>e?1:-1}):d.sort(function(a,b){var d=c.$order[f](a),e=c.$order[f](b);return d===e?0:d<e?1:-1})}void 0!==c.$take&&void 0!==c.$skip?d=d.slice(c.$skip,c.$skip+c.$take):void 0!==c.$take&&d.length>c.$take?d=d.slice(0,c.$take):c.$skip&&(d=d.slice(c.$skip,d.length));c.$some&&(d=[0<d.length]);c.$length&&(d=[d.length]);a.rawDataList=d;b.success(a)},_compile:function(a){return(new $data.storageProviders.InMemory.InMemoryCompiler(this)).compile(a)},
saveChanges:function(a,b){for(var c=0;c<b.length;c++){var e=b[c];switch(e.data.entityState){case $data.EntityState.Added:this._save_add_processPk(e);this.providerConfiguration.source[e.entitySet.tableName].push(e.data);break;case $data.EntityState.Deleted:var d=this.providerConfiguration.source[e.entitySet.tableName],f=this._save_getEntity(e,d),e=d.indexOf(f);d.splice(e,1);break;case $data.EntityState.Modified:if(e.data.changedProperties&&0<e.data.changedProperties.length){d=this.providerConfiguration.source[e.entitySet.tableName];
f=this._save_getEntity(e,d);for(d=0;d<e.data.changedProperties.length;d++){var g=e.data.changedProperties[d];!g.key&&-1<e.entitySet.elementType.memberDefinitions.getPublicMappedPropertyNames().indexOf(g.name)&&(f[g.name]=e.data[g.name])}}}}this.providerConfiguration.persistentData&&window.localStorage&&(localStorageData=window.localStorage.setItem(this.providerConfiguration.databaseName||"JayData_InMemory_Provider",JSON.stringify(this.providerConfiguration.source)));a.success()},_save_add_processPk:function(a){var b=
a.entitySet.elementType.memberDefinitions.getKeyProperties();if(1===b.length&&b[0].computed){var b=b[0],c=Container.resolveType(b.type);c===$data.Guid?a.data[b.name]=$data.Guid.NewGuid():c===$data.Integer?(c=this.providerConfiguration.source.inmemory_sequence[a.entitySet.tableName],a.data[b.name]=c+1,this.providerConfiguration.source.inmemory_sequence[a.entitySet.tableName]=c+1):Guard.raise(new Exception("Not supported data type!"))}else for(c=0;c<b.length;c++)(null===a.data[b[c].name]||void 0===
a.data[b[c].name])&&Guard.raise(new Exception("Key field must set value! Key field name without value: "+b[c].name))},_save_getEntity:function(a,b){var c=a.entitySet.elementType.memberDefinitions.getKeyProperties();entities=b.filter(function(b){for(var d=!0,f=0;f<c.length;f++)d=d&&b[c[f].name]===a.data[c[f].name];return d});1<entities&&Guard.raise(new Exception("Inconsistent storage!"));return entities[0]},getTraceString:function(a){return this._compile(a)},supportedDataTypes:{value:[$data.Integer,
...
supportedSetOperations:{value:{filter:{},map:{},length:{},forEach:{},toArray:{},single:{},some:{},take:{},skip:{},orderBy:{},orderByDescending:{},first:{}},enumerable:!0,writable:!0},fieldConverter:{value:{fromDb:{"$data.Integer":function(a){return a},"$data.Number":function(a){return a},"$data.Date":function(a){return a},"$data.String":function(a){return a},"$data.Boolean":function(a){return a},"$data.Blob":function(a){return a},"$data.Object":function(a){return void 0===a?new $data.Object:a},"$data.Array":function(a){return void 0===
a?new $data.Array:a},"$data.Guid":function(a){return"string"===typeof a?$data.parseGuid(a):a},"$data.GeographyPoint":function(a){return a?new $data.GeographyPoint(a):a},"$data.GeographyLineString":function(a){return a?new $data.GeographyLineString(a):a},"$data.GeographyPolygon":function(a){return a?new $data.GeographyPolygon(a):a},"$data.GeographyMultiPoint":function(a){return a?new $data.GeographyMultiPoint(a):a},"$data.GeographyMultiLineString":function(a){return a?new $data.GeographyMultiLineString(a):
a},"$data.GeographyMultiPolygon":function(a){return a?new $data.GeographyMultiPolygon(a):a},"$data.GeographyCollection":function(a){return a?new $data.GeographyCollection(a):a},"$data.GeometryPoint":function(a){return a?new $data.GeometryPoint(a):a},"$data.GeometryLineString":function(a){return a?new $data.GeometryLineString(a):a},"$data.GeometryPolygon":function(a){return a?new $data.GeometryPolygon(a):a},"$data.GeometryMultiPoint":function(a){return a?new $data.GeometryMultiPoint(a):a},"$data.GeometryMultiLineString":function(a){return a?
new $data.GeometryMultiLineString(a):a},"$data.GeometryMultiPolygon":function(a){return a?new $data.GeometryMultiPolygon(a):a},"$data.GeometryCollection":function(a){return a?new $data.GeometryCollection(a):a}},toDb:{"$data.Integer":function(a){return a},"$data.Number":function(a){return a},"$data.Date":function(a){return a},"$data.String":function(a){return a},"$data.Boolean":function(a){return a},"$data.Blob":function(a){return a},"$data.Object":function(a){return a},"$data.Array":function(a){return a},
"$data.Guid":function(a){return a?a.value:a},"$data.GeographyPoint":function(a){return a},"$data.GeographyLineString":function(a){return a},"$data.GeographyPolygon":function(a){return a},"$data.GeographyMultiPoint":function(a){return a},"$data.GeographyMultiLineString":function(a){return a},"$data.GeographyMultiPolygon":function(a){return a},"$data.GeographyCollection":function(a){return a},"$data.GeometryPoint":function(a){return a},"$data.GeometryLineString":function(a){return a},"$data.GeometryPolygon":function(a){return a},
"$data.GeometryMultiPoint":function(a){return a},"$data.GeometryMultiLineString":function(a){return a},"$data.GeometryMultiPolygon":function(a){return a},"$data.GeometryCollection":function(a){return a}}}}},null);$C("$data.storageProviders.InMemory.LocalStorageProvider",$data.storageProviders.InMemory.InMemoryProvider,null,{constructor:function(){this.providerConfiguration.persistentData=!0}},null);$data.StorageProviderBase.registerProvider("InMemory",$data.storageProviders.InMemory.InMemoryProvider);
$data.StorageProviderBase.registerProvider("LocalStore",$data.storageProviders.InMemory.LocalStorageProvider);
$C("$data.storageProviders.InMemory.InMemoryCompiler",$data.Expressions.EntityExpressionVisitor,null,{constructor:function(){this.provider={fieldConverter:{toDb:{"$data.Integer":function(a){return a},"$data.Number":function(a){return a},"$data.Date":function(a){return a?"new Date(Date.parse('"+a.toISOString()+"'))":null},"$data.String":function(a){return"'"+a.replace(/'/g,"''")+"'"},"$data.Boolean":function(a){return a?"true":"false"},"$data.Blob":function(a){return"string"===typeof a?"'"+a+"'":a},
"$data.Object":function(a){return JSON.stringify(a)},"$data.Array":function(a){return JSON.stringify(a)},"$data.Guid":function(a){return a?"'"+a+"'":null}}}}},compile:function(a){var b={urlText:""};this.Visit(a.expression,b);var a={},c;for(c in b)0==c.indexOf("$")&&(a[c]=b[c]);return a},VisitOrderExpression:function(a,b){this.Visit(a.source,b);b.data="";b.lambda="";Container.createInMemoryFunctionCompiler(this.provider).compile(a.selector,b);b.$order=b.$order||[];var c=new Function(b.lambda,"return "+
b.data+";");c.ASC="OrderBy"==a.nodeType;b.$order.push(c);b.data="";b.lambda=""},VisitIncludeExpression:function(a,b){this.Visit(a.source,b);b.$include=b.$include||[];0>b.$include.indexOf(a.selector.value)&&b.$include.push(a.selector.value)},VisitPagingExpression:function(a,b){this.Visit(a.source,b);b["$"+a.nodeType.toLowerCase()]=a.amount.value},VisitProjectionExpression:function(a,b){this.defaultFunctionCompiler(a,b,"$map")},VisitFilterExpression:function(a,b){this.defaultFunctionCompiler(a,b,"$filter")},
VisitSomeExpression:function(a,b){this.defaultFunctionCompiler(a,b,"$some")},VisitEveryExpression:function(a,b){this.defaultFunctionCompiler(a,b,"$every")},VisitCountExpression:function(a,b){this.Visit(a.source,b);b.$length=!0},VisitServiceOperationExpression:function(a,b){b.$serviceOperation={name:a.cfg.serviceName,params:a.params}},defaultFunctionCompiler:function(a,b,c){this.Visit(a.source,b);b.data="";b.lambda="";Container.createInMemoryFunctionCompiler(this.provider).compile(a.selector,b);b[c]=
new Function(b.lambda,"return "+b.data+";");b.data="";b.lambda=""}},{});

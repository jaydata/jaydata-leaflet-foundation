...
{$type:$data.Array,$selector:"json:"+b.FromPropertyName,$item:{$type:$data.ObjectID,$value:function(b,a){var c=Container.resolveName(b.$type),d=this.context.storageProvider.fieldConverter.fromDb;return d&&d[c]?d[c](a):new (Container.resolveType(c))(a)}}}):$data.typeSystem.extend(a.modelBinderConfig,{$type:b.ToType,$source:b.FromPropertyName})}},_addComplexTypeProperties:function(b,a){var c=this;b.forEach(function(b){a.selectModelBinderProperty(b.FromPropertyName);c._addComplexType(b,a);a.popModelBinderProperty()},
...
b.operation,$data.Expressions.MemberInfoExpression);this.Visit(b.source.selector,a);var c=b.operation.memberDefinition,d=c.mapTo||c.name,g=0;(c.parameters||[{name:"@expression"}]).map(function(a){return"@expression"===a.name?b.source:b.parameters[g++]}).forEach(function(b){this.Visit(b,a)},this);var f,h;switch(d){case "contains":f="$regex";h=a.value.replace(/([.?*+^$[\]\\(){}|-])/g,"\\$1");break;case "startsWith":f="$regex";h="^"+a.value.replace(/([.?*+^$[\]\\(){}|-])/g,"\\$1");break;case "endsWith":f=
"$regex",h=a.value.replace(/([.?*+^$[\]\\(){}|-])/g,"\\$1")+"$"}a.unary===$data.Expressions.ExpressionType.Not&&(h="^((?!"+h+").)*$");f&&h&&(a.cursor instanceof Array?(c={},c[a.field]={},c[a.field][f]=h,a.cursor.push(c)):(a.cursor[a.field]={},a.cursor[a.field][f]=h))},VisitConstantExpression:function(b,a){var c=Container.getTypeName(b.value);a.valueType=c;a.value=b.value},VisitEntityExpression:function(b,a){a.entityType=b.entityType;this.Visit(b.source,a)},VisitEntitySetExpression:function(b,a){this.Visit(b.source,
a);b.selector instanceof $data.Expressions.AssociationInfoExpression&&(this.Visit(b.selector,a),a.data+="/")},VisitFrameOperationExpression:function(b,a){this.Visit(b.source,a);Guard.requireType("expression.operation",b.operation,$data.Expressions.MemberInfoExpression);var c=b.operation.memberDefinition;a.data+=c.mapTo||c.name;a.data+="(";for(var d=0,g=(c.parameters||[{name:"@expression"}]).map(function(a){return"@expression"===a.name?b.source:b.parameters[d++]}),f=0;f<g.length;f++){var h=g[f];if(h.value instanceof
$data.Queryable){var e=new c.frameType(h.value.expression),h=(new $data.Expressions.QueryExpressionCreator(h.value.entityContext)).Visit(e),e={data:""};(new $data.storageProviders.mongoDB.mongoDBWhereCompiler(this.provider,!0)).compile(h,e);a.data+=e.lambda+": "+e.data}}a.data+=")"}});
...
".");a.data+=b.memberDefinition.computed?"_id":b.memberName}});
...
{safe:!1})).open(function(c,d){if(c)b.error(c);else{var g=function(c,d){var e=0,g=function(c){0>=--e&&(b.success(a.context),c.close())},r;for(r in a.context._entitySetReferences)a.context._entitySetReferences.hasOwnProperty(r)&&e++;r=Object.keys(a.context._entitySetReferences);if(!r.length)return g(d);r.forEach(function(c){a.context._entitySetReferences.hasOwnProperty(c)&&d.collectionNames({namesOnly:!0},function(f,e){e=e.map(function(b){return b.slice(b.lastIndexOf(".")+1)});switch(a.providerConfiguration.dbCreation){case $data.storageProviders.DbCreationType.DropAllExistingTables:0<=
e.indexOf(a.context._entitySetReferences[c].tableName)?d.dropCollection(a.context._entitySetReferences[c].tableName,function(f){f?b.error(f):a.context._entitySetReferences[c].tableOptions?d.createCollection(a.context._entitySetReferences[c].tableName,a.context._entitySetReferences[c].tableOptions,function(a){a?b.error(a):g(d)}):g(d)}):0>e.indexOf(a.context._entitySetReferences[c].tableName)&&a.context._entitySetReferences[c].tableOptions?d.createCollection(a.context._entitySetReferences[c].tableName,
a.context._entitySetReferences[c].tableOptions,function(a){a?b.error(a):g(d)}):g(d);break;default:0>e.indexOf(a.context._entitySetReferences[c].tableName)&&a.context._entitySetReferences[c].tableOptions?d.createCollection(a.context._entitySetReferences[c].tableName,a.context._entitySetReferences[c].tableOptions,function(a){a?b.error(a):g(d)}):g(d)}})})};a.providerConfiguration.username?d.authenticate(a.providerConfiguration.username,a.providerConfiguration.password||"",function(a,c){a?b.error(a):
c&&g(a,d)}):g(c,d)}})},executeQuery:function(b,a){var c=this,a=$data.typeSystem.createCallbackSetting(a),d=b.context.getEntitySetFromElementType(b.defaultType);(new $data.storageProviders.mongoDB.mongoDBCompiler).compile(b);(new this.driver.Db(this.providerConfiguration.databaseName,this._getServer(),{safe:!1})).open(function(g,f){if(g)a.error(g);else{var h=new c.driver.Collection(f,d.tableName),e=b.find,o=function(d,e){d?a.error(d):(b.rawDataList=e instanceof Array?e:[{cnt:e}],b.context=c.context,
a.success(b),f.close())},r=function(){switch(b.expression.nodeType){case $data.Expressions.ExpressionType.Count:h.find(e.query,e.options).count(o);break;case $data.Expressions.ExpressionType.BatchDelete:h.remove(e.query,{safe:!0},o);break;default:h.find(e.query,e.options).toArray(o)}};c.providerConfiguration.username?f.authenticate(c.providerConfiguration.username,c.providerConfiguration.password,function(b,c){b?a.error(b):c&&r()}):r()}})},_typeFactory:function(b,a,c){b=Container.resolveName(b);return c&&
c[b]?c[b](a):new (Container.resolveType(b))(a)},_saveCollections:function(b,a){var c=this,d=0,g=this._getServer(),f=0,h=function(b){0>=--f&&b()},e=function(a,e,f){for(var h=[],g=0;g<e.insertAll.length;g++){for(var k=e.insertAll[g],m=Container.resolveType(k.type).memberDefinitions.getPublicMappedProperties(),l=0;l<m.length;l++){var i=m[l];if(i.concurrencyMode===$data.ConcurrencyMode.Fixed)k.data[i.name]=0;else if(i.computed)"string"===typeof k.data[i.name]&&(k.data._id=c._typeFactory(i.type,k.data[i.name],
c.fieldConverter.toDb));else{if(Container.resolveType(i.type)===$data.Array&&i.elementType&&Container.resolveType(i.elementType)===$data.ObjectID){k.data[i.name]=c._typeFactory(i.type,k.data[i.name],c.fieldConverter.toDb);var q=k.data[i.name];if(q)for(var j=0;j<q.length;j++)q[j]=c._typeFactory(i.elementType,q[j],c.fieldConverter.toDb)}else k.data[i.name]=c._typeFactory(i.type,k.data[i.name],c.fieldConverter.toDb);k.data[i.name]&&k.data[i.name].initData&&(k.data[i.name]=k.data[i.name].initData)}}h.push(k.data)}f.insert(h,
{safe:!0},function(h,g){if(h)b.error(h);else{for(var i=0;i<g.length;i++)for(var k=g[i],j=e.insertAll[i],n=Container.resolveType(j.type).memberDefinitions.getPublicMappedProperties(),l=0;l<n.length;l++){var m=n[l];j.entity[m.name]=c._typeFactory(m.type,k[m.computed?"_id":m.name],c.fieldConverter.fromDb)}d+=g.length;e.removeAll&&e.removeAll.length?r(a,e,f):e.updateAll&&e.updateAll.length?o(a,e,f):s(a,d)}})},o=function(a,e,g){f=e.updateAll.length;for(var n=0;n<e.updateAll.length;n++){for(var p=e.updateAll[n],
...
1;else if(!j.computed&&"undefined"!==typeof p.entity[j.name])if(Container.resolveType(j.type)===$data.Array&&j.elementType&&Container.resolveType(j.elementType)===$data.ObjectID){q[j.name]=c._typeFactory(j.type,p.entity[j.name],c.fieldConverter.toDb);var o=q[j.name];if(o)for(i=0;i<o.length;i++)o[i]=c._typeFactory(j.elementType,o[i],c.fieldConverter.toDb)}else q[j.name]=c._typeFactory(j.type,p.entity[j.name],c.fieldConverter.toDb)}(function(e){g.update(k,{$set:q},{safe:!0},function(i,j){if(i)b.error(i);
else if(j){d++;for(var l=Container.resolveType(e.type).memberDefinitions.getPublicMappedProperties(),n=0;n<l.length;n++){var m=l[n];m.concurrencyMode===$data.ConcurrencyMode.Fixed&&e.entity[m.name]++}h(function(){s(a,d)})}else f--,g.find({_id:k._id},{}).toArray(function(i,g){if(i)b.error(i);else{for(var f=g[0],k=Container.resolveType(e.type).memberDefinitions.getPublicMappedProperties(),j=0;j<k.length;j++){var n=k[j];e.entity[n.name]=c._typeFactory(n.type,f[n.computed?"_id":n.name],c.fieldConverter.fromDb)}h(function(){s(a,
d)})}})})})(p)}},r=function(a,e,g){f=e.removeAll.length;for(var n=0;n<e.removeAll.length;n++){for(var p=e.removeAll[n],k=Container.resolveType(p.type).memberDefinitions.getKeyProperties(),m=0;m<k.length;m++){var l=k[m];p.data[l.computed?"_id":l.name]=c.fieldConverter.toDb[Container.resolveName(Container.resolveType(l.type))](p.entity[l.name])}k=Container.resolveType(p.type).memberDefinitions.getPublicMappedProperties();for(m=0;m<k.length;m++)l=k[m],l.key||delete p.data[l.name];g.remove(p.data,{safe:!0},
function(c,n){c?b.error(c):(n?d++:f--,h(function(){e.updateAll&&e.updateAll.length?o(a,e,g):s(a,d)}))})}},t=Object.keys(a),s=function(d,g){if(t.length){var f=t.pop();if(a.hasOwnProperty(f)){var h=a[f],f=new c.driver.Collection(d,f);h.insertAll&&h.insertAll.length?e(d,h,f):h.removeAll&&h.removeAll.length?r(d,h,f):h.updateAll&&h.updateAll.length?o(d,h,f):(b.success(0),d.close())}}else b.success(g),d.close()};(new this.driver.Db(this.providerConfiguration.databaseName,g,{safe:!1})).open(function(a,d){a?
b.error(a):c.providerConfiguration.username?d.authenticate(c.providerConfiguration.username,c.providerConfiguration.password,function(a,c){a?b.error(a):c&&s(d)}):s(d)})},saveChanges:function(b,a){if(a.length){for(var c=this.buildIndependentBlocks(a),d=[],g={},f=0;f<c.length;f++)for(var h=0;h<c[f].length;h++){d.push(c[f][h].data);var e=g[c[f][h].entitySet.name];e||(e={},g[c[f][h].entitySet.name]=e);var o={entity:c[f][h].data,data:this.save_getInitData(c[f][h],d),type:Container.resolveName(c[f][h].data.getType())};
switch(c[f][h].data.entityState){case $data.EntityState.Unchanged:continue;case $data.EntityState.Added:e.insertAll||(e.insertAll=[]);e.insertAll.push(o);break;case $data.EntityState.Modified:e.updateAll||(e.updateAll=[]);e.updateAll.push(o);break;case $data.EntityState.Deleted:e.removeAll||(e.removeAll=[]);e.removeAll.push(o);break;default:Guard.raise(new Exception("Not supported Entity state"))}}this._saveCollections(b,g)}else b.success(0)},save_getInitData:function(b,a){b.physicalData=this.context._storageModel.getStorageModel(b.data.getType()).PhysicalType.convertTo(b.data,
...
b?new $data.Array:b},"$data.ObjectID":function(b){return b?(new Buffer(b.toString(),"ascii")).toString("base64"):b},"$data.GeographyPoint":function(b){return b?new $data.GeographyPoint(b):b},"$data.GeographyLineString":function(b){return b?new $data.GeographyLineString(b):b},"$data.GeographyPolygon":function(b){return b?new $data.GeographyPolygon(b):b},"$data.GeographyMultiPoint":function(b){return b?new $data.GeographyMultiPoint(b):b},"$data.GeographyMultiLineString":function(b){return b?new $data.GeographyMultiLineString(b):
b},"$data.GeographyMultiPolygon":function(b){return b?new $data.GeographyMultiPolygon(b):b},"$data.GeographyCollection":function(b){return b?new $data.GeographyCollection(b):b},"$data.GeometryPoint":function(b){return b?new $data.GeometryPoint(b):b},"$data.GeometryLineString":function(b){return b?new $data.GeometryLineString(b):b},"$data.GeometryPolygon":function(b){return b?new $data.GeometryPolygon(b):b},"$data.GeometryMultiPoint":function(b){return b?new $data.GeometryMultiPoint(b):b},"$data.GeometryMultiLineString":function(b){return b?
new $data.GeometryMultiLineString(b):b},"$data.GeometryMultiPolygon":function(b){return b?new $data.GeometryMultiPolygon(b):b},"$data.GeometryCollection":function(b){return b?new $data.GeometryCollection(b):b},"$data.Guid":function(b){return b?$data.parseGuid(b):b}},toDb:{"$data.Integer":function(b){return b},"$data.Number":function(b){return b},"$data.Date":function(b){return b},"$data.String":function(b){return b},"$data.Boolean":function(b){if("string"===typeof b)switch(b){case "true":case "true":case "TRUE":case "yes":case "Yes":case "YES":return!0;
default:return!1}return null===b||void 0===b?null:!!b},"$data.Blob":function(b){return b},"$data.Object":function(b){return b},"$data.Array":function(b){return b},"$data.ObjectID":function(b){return b&&"string"===typeof b?new $data.mongoDBDriver.ObjectID.createFromHexString((new Buffer(b,"base64")).toString("ascii")):b},"$data.GeographyPoint":function(b){return b},"$data.GeographyLineString":function(b){return b},"$data.GeographyPolygon":function(b){return b},"$data.GeographyMultiPoint":function(b){return b},
"$data.GeographyMultiLineString":function(b){return b},"$data.GeographyMultiPolygon":function(b){return b},"$data.GeographyCollection":function(b){return b},"$data.GeometryPoint":function(b){return b},"$data.GeometryLineString":function(b){return b},"$data.GeometryPolygon":function(b){return b},"$data.GeometryMultiPoint":function(b){return b},"$data.GeometryMultiLineString":function(b){return b},"$data.GeometryMultiPolygon":function(b){return b},"$data.GeometryCollection":function(b){return b},"$data.Guid":function(b){return b?
b.value:b}}}}},{isSupported:{get:function(){return!$data.mongoDBDriver?!1:!0},set:function(){}}});$data.storageProviders.mongoDB.mongoDBProvider.isSupported&&$data.StorageProviderBase.registerProvider("mongoDB",$data.storageProviders.mongoDB.mongoDBProvider);try{"undefined"===typeof navigator&&(navigator=window.navigator=require("navigator"))}catch(err){}"undefined"===typeof btoa&&(btoa=window.btoa=function(b){return(new Buffer(b,"ascii")).toString("base64")});
$data.Class.define("$data.storageProviders.mongoDB.mongoDBProvider.ClientObjectID",null,null,{constructor:function(){var b=Math.floor((new Date).getTime()/1E3).toString(16),a=btoa(navigator?navigator.userAgent:"nodejs"),c=(a.charCodeAt(0)+a.charCodeAt(1)).toString(16)+(a.charCodeAt(2)+a.charCodeAt(3)).toString(16)+(a.charCodeAt(4)+a.charCodeAt(5)).toString(16),d=("0000"+Math.floor(Math.random()*65535).toString(16)).slice(-4),g=("000000"+(++$data.storageProviders.mongoDB.mongoDBProvider.ClientObjectID.idSeed).toString(16)).slice(-6);
this.toString=this.toLocaleString=this.valueOf=function(){return btoa(b+c+d+g)}},value:{value:null}},{idSeed:{value:Math.floor(255*Math.random())}});
